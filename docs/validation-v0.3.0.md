# Deep Fry 0.3.0 validation

This release adds genuine JPEG snapshot export, a default stereo display, and
bundled IBM Plex interface fonts. The audio algorithm, automatable parameter IDs,
...
| Native editor and artifact run | 1,384,742 checks passed |
...
| Mac and Windows version/font embedding checks | 0.3.0 and all three complete TTF payloads verified |

Thirteen native editor PNGs and the actual JPEG export were inspected, including
stereo, L/R solo, selected right tile, idle, help, effect OFF, open menus, and
896×640 minimum-size layouts. The README screenshot shows this release.
...
PE32+ AMD64 binaries with version 0.3.0.0, static compiler runtime and system-DLL
imports. The VST3 manifest reports 0.3.0 with unchanged plugin class identifiers.

Local evidence is in `.context/preview-030/`, `.context/artifacts-030.log`,
`.context/ctest-030-macos.log`, `.context/pluginval-030-*.log`, and
`.context/windows-cross/*030*`. Build logs ending in `polish.log` include the
final minimum-size caption adjustments. These private artifacts are excluded
...
build-public/DeepFryVerify_artefacts/Release/DeepFryVerify --artifacts .context/preview-030
