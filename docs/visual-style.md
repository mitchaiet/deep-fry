...
- **One printed surface.** Cream paper, fine rules, flat square controls, and a red masthead hold the interface together. The tilted brand stamp retains its offset shadow. Texture is fixed and cached. Control text and values stay sharp.
...
- **Controls work like an export dialog.** Horizontal sliders have slim rectangular handles, ticks, units, editable values, and visible focus outlines. Yellow marks selected presets and active toggles. Effect has fixed ON / OFF choices: selected ON is yellow, selected OFF is ink with white text. The labels stay in place so the current state is explicit.
- **Image settings have their own language.** Labeled View and Palette dropdowns use square paper fields and arrows. View chooses Output (what you hear) or JPEG only (before mix); Palette chooses Colour or Grayscale. The footer reads "VIEW + PALETTE: IMAGE ONLY" in high-contrast text. Save JPEG and Freeze sit beside the menus; Stereo / L / R controls the channel layout.
