...
```powershell
$expected = ((Get-Content "Deep-Fry-0.2.1-Windows-x64.zip.sha256") -split '\s+')[0]
$actual = (Get-FileHash "Deep-Fry-0.2.1-Windows-x64.zip" -Algorithm SHA256).Hash
...
Install **Visual Studio 2022**, its **Desktop development with C++** workload and a Windows SDK, plus **CMake 3.24+**. Download the shared `Deep-Fry-0.2.1-source.tar.gz` asset from the [release](https://github.com/mitchaiet/deep-fry/releases/tag/v0.2.1). The build uses its included JUCE archive without downloading dependencies.

From PowerShell in the folder containing the source download:

```powershell
cmake -E tar xzf .\Deep-Fry-0.2.1-source.tar.gz
Set-Location .\Deep-Fry-0.2.1-source
