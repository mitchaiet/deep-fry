...
Choose **Meme**, play a loop, and lower **JPEG Quality** to introduce more artifacts. Increase **Fry** for contrast and sharpening distortion. For a before/after comparison, choose **Effect OFF** to hear the original, then **ON** to hear the effect; adjust **Output** while ON to match the perceived volume. **Deep fried** and **Lost cause** are more extreme starting points.

The **View** and **Palette** menus change the image only. Leave View on **Output (what you hear)** to follow the plug-in's audible output, or select **JPEG only (before mix)** to inspect the compression stage. Palette chooses **Colour** or **Grayscale** for both panels.
...
The **Palette** menu switches both panels between **Colour** and **Grayscale** so their amplitudes can be compared using the same scale. Colour is the default; the audio codec itself always operates on grayscale values.
...
- **Mix, Output, and Effect ON/OFF** affect newly captured OUTPUT tiles. JPEG ONLY stays available for inspecting the compression stage independently.
- **IN / OUT** show actual peak levels across the active audio channels, with meter decay. OUT reflects Mix, Output gain, and Effect ON/OFF. A signal on only one stereo channel can move these meters while the other channel's picture remains flat.
